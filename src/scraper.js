...
async function getEpisodeFeedWithBrowser(pageUrl) {
  let browser;
  try {
    ...
    const collect = async () => {
      const data = await page.evaluate(() => {
        const out = [];
        document.querySelectorAll(".mono_type").forEach(el => {
          const date = el.getAttribute("data-ts") || "";
      ...
      for (const item of data) {
        ...
        const playerUrl = abs(item.src, page.url());
    ...
    return [...discovered].map(x => JSON.parse(x));
  } catch {
    return [];
  } finally {
    if (browser) {
...
async function getNovelaById(id) {
  ...
  // The episode list is generated by the site's client-side JavaScript.
  // Use a normal headless browser as a fallback so the public JS flow can run.
  if (!eps.length) {
    eps = await getEpisodeFeedWithBrowser(pageUrl);
  }
